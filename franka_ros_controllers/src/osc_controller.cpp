...
#include <franka_ros_controllers/osc_controller.h>
...
bool OSCController::init(hardware_interface::RobotHW* robot_hw,
                                        ros::NodeHandle& node_handle) {
  std::vector<double> cartesian_stiffness_vector;
  std::vector<double> cartesian_damping_vector;

  sub_equilibrium_pose_ = node_handle.subscribe(
      "/equilibrium_pose", 20, &OSCController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());
  stiffness_params_ = node_handle.subscribe(
      "/impedance_stiffness", 20, &OSCController::stiffnessParamCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());
 
  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("OSCController: Could not read parameter arm_id");
    return false;
  }
  std::vector<std::string> joint_names;
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "OSCController: Invalid or no joint_names parameters provided, "
        "aborting controller init!");
    return false;
  }
  if (!node_handle.getParam("stiffness_gains", stiffness_gains_) || stiffness_gains_.size() != 6) {
    ROS_ERROR(
        "OSCController:  Invalid or no cartesian_stiffness_ parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "OSCController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "OSCController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "OSCController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "OSCController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "OSCController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "OSCController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
...
void OSCController::starting(const ros::Time& /*time*/) {
...
void OSCController::update(const ros::Time& /*time*/,
...
Eigen::Matrix<double, 7, 1> OSCController::saturateTorqueRate(
...
void OSCController::stiffnessParamCallback(
...
void OSCController::equilibriumPoseCallback(
...
PLUGINLIB_EXPORT_CLASS(franka_ros_controllers::OSCController,
